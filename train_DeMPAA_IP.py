...
parser.add_argument('--max_count', type=int, default=2000, help='The max number of iterations to find adversarial example')
...
nps_calculator = NPSCalculator("non_printability/30values.txt", image_size).cuda()
...
def attack(x, patch, data_shape,patch_shape,image_size,x_label,pred):
    ...
    for i in range(3):
        ...
        x_patch = np.zeros((1, 3, image_size, image_size))
        x_patch = gauss_noise(x_patch, 0, 75)
        ...
        if not is_allow:
            ...
            adv_x = torch.mul(1, x) + torch.mul(mask, x_patch)
            adv_x = torch.clamp(adv_x, min_out, max_out)
            ...
            while x_perc_t > 10 :
                ...
                tv_loss1 = tv_loss(adv_x)
                optim.zero_grad()
                d_map = ciede2000_diff(rgb2lab_diff(x, device), rgb2lab_diff(adv_x, device), device).unsqueeze(1)
                color_dis = torch.norm(d_map.view(1, -1), dim=1)
                color_loss = color_dis.sum()
                Loss = x_perc + color_loss * 0.005 + tv_loss1 * 10000 / 4
                Loss.backward()
                optim.step()
